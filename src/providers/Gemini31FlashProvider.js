...
class Gemini31FlashProvider extends BaseProvider {
  constructor() {
    super({ name: 'gemini31flash', supportsGeneration: true, supportsEditing: true });
  }

  async _callAPI({ prompt, images = [], aspectRatio, imageSize }) {
    const url = `${config.geminiBaseUrl}/v1beta/models/gemini-3.1-flash-image-preview:generateContent`;
    ...
    if (!candidates?.length) {
      throw new Error('No candidates returned from Gemini 3.1 Flash API');
    }

    const content = candidates[0]?.content;
    if (!content?.parts) {
      throw new Error('No content parts in Gemini 3.1 Flash response');
    }

    const imagePart = content.parts.find((p) => p.inlineData?.data);
    if (!imagePart) {
      throw new Error('No image data in Gemini 3.1 Flash response');
    }
...
module.exports = Gemini31FlashProvider;
